...
public static class CacheManagerContainer
{
    ...
    /// <exception cref="ArgumentException">Thrown if the container has not been initialized by calling <see cref="Initialize"/>.</exception>
    public static ICacheManager Resolve()
    {
        if (_serviceProvider is null)
        {
            throw new ArgumentException("CacheManagerContainer is not initialized. Call Initialize() with a valid IServiceProvider.");
        }
